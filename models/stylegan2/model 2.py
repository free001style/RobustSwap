...
from models.stylegan2.op import upfirdn2d, conv2d_gradfix
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style):
        ...
        weight = self.scale * self.weight * style  # [1, C_out, C_in, kh, kw]  * [bs,1,512,1,1]
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, mask, noise=None):
        if not self.mask_op:
            ...
            comp_aware_out = torch.zeros(bs, self.conv.out_channel, H_out, W_out, device=input.device)
...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, mask, skip=None):
        if not self.mask_op:
            ...
            bs, _, H, W = input.size()
            segmap = F.interpolate(mask, size=(H, W), mode='nearest')  # one-hot, [bs,#seg_cls,H,W]
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_rgbs.append(
                ToRGB(
                    ...
                    mask_op=False if self.remaining_layer_idx != 17 and i >= (
                            2 + self.remaining_layer_idx // 2) else True,  # 这里一定是大于等于
                    # mask_op = True
                )
            )
